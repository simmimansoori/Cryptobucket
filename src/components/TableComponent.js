...
const TableComponent = () => {
  ...
  return (
    ...
    <>
         <div className='flex flex-col mt-9 border border-gray-300 rounded '> 
        ...
        cryptoData.map(data => {
            return (
              ...
              >
                ...
                <td className={
                    data.price_change_percentage_1h_in_currency > 0 ? 'text-green-400 py-4' : 'py-4 text-red-400'
                }>{Number(data.price_change_percentage_1h_in_currency).toFixed(2)}</td>
                <td className={
                    data.price_change_percentage_24h_in_currency > 0 ? 'text-green-400 py-4' : 'py-4 text-red-400'
                }>{Number(data.price_change_percentage_24h_in_currency).toFixed(2)}</td>
                <td className={
                    data.price_change_percentage_7d_in_currency > 0 ? 'text-green-400 py-4' : 'py-4 text-red-400'
...
<div className='flex items-center justify-between mt-4 capitalize h-[2rem]'>
  <span>Data provided by <a className='text-cyan-300' href="http://www.coingecko.com" target={'_blank'} rel='noreferrer'>CoinGecko</a></span>
 <Pagination /> 
</div>
    </>
  )
